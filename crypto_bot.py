...
Automates extraction of the top 10 crypto assets from CoinGecko,
calculates RSI for momentum analysis, provides simple Buy/Hold/Sell
recommendations, and saves styled Excel reports with visuals.
...
class CryptoMarketPipeline:
    def __init__(self, report_file):
        ...
        # Parameters: top 10 USD assets with sparkline
        self.params = {
            "vs_currency": "usd",
        ...
        # HTTP session with retries for reliability
    ...
    def _process_data(self, raw_data, total_mcap):
        ...
        df["Recommendation"] = df.apply(lambda row: self._get_recommendation(row["RSI"], row["price_change_percentage_24h"]), axis=1)
    ...
    def run_cycle(self):
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Running market update...")
